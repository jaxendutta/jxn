...
import { getGlyphs, mignovaFont, csDeviousStippledFont, serifFont } from "@/lib/fonts";
...
const NAME_WORDS = ["JXNCO"];

// Flat character sequence across the wordmark
const ALL_CHARS = NAME_WORDS.join("");

// Flat index across all letters
function buildLetterStates(): LetterState[] {
    return ALL_CHARS.split("").map((char) => ({
        char,
        glyphIndex: -1,
        isActive: false,
        idleBleeding: false,
...
export default function HeroInteraction() {
    ...
    const activeTimersRef = useRef<Map<number, ReturnType<typeof setTimeout>>>(
        new Map(),
    );
    const idleTimerRef = useRef<ReturnType<typeof setInterval> | null>(null);
    const cycleTimersRef = useRef<Map<number, ReturnType<typeof setInterval>>>(
        new Map(),
    );
    ...
    const updateLetter = useCallback(
        (index: number, patch: Partial<LetterState>) => {
            setLetters((prev) => {
                const next = [...prev];
    ...
    const triggerActivation = useCallback(
        (index: number, speed = 60) => {
            const glyphs = getGlyphs(ALL_CHARS[index] ?? "");
            let step = 0;
                ...
                [-2, -1, 1, 2].forEach((offset) => {
                    ...
                    setTimeout(() => {
                        setLetters((prev) => {
                            ...
                            const glyphs = getGlyphs(ALL_CHARS[ni] ?? "");
                            const next = [...prev];
                            next[ni] = {
                                ...next[ni],
                                idleBleeding: true,
                                glyphIndex: Math.floor(
    ...
    // ── Wordmark letter renderer — real char + sigil overlay ─────────────
    const renderGlyphLetter = (char: string, fi: number) => {
